...
export default function WaterDropAdventure({ game, username, onFinish }) {
    ...
    <div style={fullScreenStyle}>
      ...
      <div style={cardStyle}>
        ...
        <p style={{ color: "#94a3b8", fontSize: 13, margin: "0 0 20px", textAlign: "center", maxWidth: 300 }}>
          Answer questions correctly to fill the river and move your character along the path.
        </p>
        <div style={pillsRow}>
  ...
  return (
    <div style={fullScreenStyle}>
      ...
      <div style={{ background: "linear-gradient(180deg, #0c1f3a 0%, #0a3d62 100%)", padding: "12px 16px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ background: "#1e3a5f", borderRadius: 20, height: 20, position: "relative", overflow: "visible" }}>
          <div style={{ width: `${waterLevel}%`, height: "100%", background: "linear-gradient(90deg, #0c4a6e, #0ea5e9, #38bdf8)", borderRadius: 20, transition: "width 0.6s ease", position: "relative" }}>
          ...
          <div style={{
            ...
            animation: dropAnimation ? "wbounce 0.5s ease" : "wfloat 2s ease-in-out infinite",
          }}>💧</div>
        </div>
      ...
      <div style={{ background: "#0f2040", border: "1px solid #1e3a5f", borderRadius: 12, padding: "14px 18px", margin: "10px 16px", textAlign: "center" }}>
        <p style={{ fontSize: 17, fontWeight: 700, color: "#f1f5f9", margin: 0, lineHeight: 1.4 }}>{currentQ.questionText}</p>
        {currentQ.hint && !answered && <p style={{ fontSize: 12, color: "#64748b", margin: "6px 0 0" }}>💡 Hint: {currentQ.hint}</p>}
      </div>
...
const fullScreenStyle = { minHeight: "100vh", background: "#060e1a", display: "flex", flexDirection: "column", fontFamily: "'Nunito', sans-serif" };
const cardStyle = { margin: "auto", maxWidth: 420, width: "100%", padding: "32px 24px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" };
const avatarStyle = (size) => ({ width: size, height: size, borderRadius: "50%", border: "3px solid #0ea5e9", background: "#0f2040", flexShrink: 0 });
const titleStyle = { fontSize: 30, color: "#f1f5f9", margin: "12px 0 8px", fontFamily: "'Fredoka One', cursive" };
...
const btnStyle = (c) => ({ padding: "14px 32px", background: c, color: "#fff", fontSize: 16, fontWeight: 800, border: "none", borderRadius: 16, cursor: "pointer", fontFamily: "'Nunito', sans-serif", boxShadow: `0 4px 20px ${c}66` });
