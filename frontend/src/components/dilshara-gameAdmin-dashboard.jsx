...
// ── Which subTypes are "non-quiz" (auto-generate their own content) ─────────
...
const GAME_TYPE_INFO = {
  germcatcher: {
    ...
    contentSource: "Word-bank (auto)",
    color: "#EC4899",
  },
  waterdrop: {
    emoji: "💧",
    label: "Water Drop Adventure",
  ...
  memory: {
    ...
    contentSource: "Card library (auto)",
    color: "#A855F7",
  },
};
...
// ─────────────────────────────────────────────────────────────────────────────
//  QUESTION EDITOR  (only used for quiz subType)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  NON-QUIZ STEP 2  — shown when subType is germcatcher / waterdrop / memory
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <div style={{ ...sectionStyle, borderColor: info.color + "55" }}>
        ...
        <div style={{
          background: "#0B1E33", borderRadius: 10, padding: 16, marginBottom: 16,
          border: "1px solid #1E3A56",
        }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#7BAED4", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 8 }}>
            How kids will play
          </div>
        ...
        {/* Content source info */}
        <div style={{
          background: info.color + "11", border: `1px solid ${info.color}44`,
          ...
          {form.subType === "memory" ? (
            <p style={{ fontSize: 13, color: "#94A3B8", margin: 0, lineHeight: 1.5 }}>
              Memory Match uses a built-in card library organised by topic. The game will automatically pick the right word-icon pairs for <strong style={{ color: "#F0F6FF" }}>{form.topicName}</strong>. No extra setup needed.
            </p>
          ) : (
            <p style={{ fontSize: 13, color: "#94A3B8", margin: 0, lineHeight: 1.5 }}>
              This game generates its content from a topic word-bank at play time. It uses the topic <strong style={{ color: "#F0F6FF" }}>{form.topicName}</strong> to show relevant words, facts, and answers automatically — no manual questions required.
            </p>
          )}
        </div>
      </div>
      ...
      <button onClick={onSave} disabled={loading} style={{
        padding: "12px 32px",
        background: loading ? "#1A3050" : "#1D9E75",
...
function CreateGameForm({ token, onSuccess }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) { setGenerateMsg("⚠️ " + (data.message || "Failed to generate questions.")); return; }
  ...
  // ── Save for non-quiz types (no questions array needed) ───────────────────
  const handleSaveNonQuiz = async () => {
    ...
    try {
      const payload = {
        ...form,
        questions: [],           // empty — game generates its own content
    ...
    <div style={{ maxWidth: 680 }}>
      ...
      {step === 1 && (
        <>
          ...
          {/* 3. Game Experience */}
          <div style={sectionStyle}>
            <h3 style={{ fontSize: 13, fontWeight: 700, color: "#7BAED4", margin: "0 0 6px", textTransform: "uppercase", letterSpacing: "0.8px" }}>
              3. Game Experience (How kids will play)
            </h3>
            <p style={{ fontSize: 12, color: "#4A6A88", margin: "0 0 14px" }}>
              Standard Quiz needs you to add questions. The other 3 games auto-generate their own content.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                { id: "quiz",        label: "📝 Standard Quiz",       desc: "You write the questions — best for ages 11–15",  note: "Manual questions required", noteColor: "#EF9F27" },
                { id: "germcatcher", label: "🦠 Germ Catcher",        desc: "Tap germ bubbles — ages 5–10",                   note: "Auto-content · No questions needed", noteColor: "#1D9E75" },
                { id: "waterdrop",   label: "💧 Water Drop Adventure", desc: "Guide a water drop — ages 5–10",                 note: "Auto-content · No questions needed", noteColor: "#1D9E75" },
                { id: "memory",      label: "🃏 Memory Match",         desc: "Flip & match cards — ages 5–10",                 note: "Auto-content · No questions needed", noteColor: "#1D9E75" },
              ].map(exp => (
          ...
          <div style={sectionStyle}>
            ...
            <div>
              ...
              <textarea value={form.description} onChange={e => handleField("description", e.target.value)}
                rows={2} placeholder="Brief description of this game..."
                style={{ ...inputStyle, resize: "vertical" }} />
            </div>
          </div>
      ...
      {step === 2 && !isNonQuiz && (
        <>
          ...
          <div style={{ ...sectionStyle, borderColor: "#1A4A6A" }}>
            ...
            <p style={{ fontSize: 12, color: "#4A6A88", margin: "0 0 14px" }}>
              Fetches {config.questionCount} {form.difficulty} questions automatically. Review before saving.
            </p>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {questions.length === 0 && (
              <div style={{
                ...
                borderRadius: 10, padding: 24, textAlign: "center", color: "#4A6A88", fontSize: 13,
              }}>No questions yet. Use "Generate" above or "Add Question Manually".</div>
            )}
            {questions.map((q, i) => (
...
// ─────────────────────────────────────────────────────────────────────────────
//  GAME CARD  — shows subType badge
// ─────────────────────────────────────────────────────────────────────────────
function GameCard({ game, onDelete }) {
  const diff = { easy: "#1D9E75", medium: "#EF9F27", hard: "#E05252" };
  const subTypeEmoji = { quiz: "📝", germcatcher: "🦠", waterdrop: "💧", memory: "🃏" };
  return (
