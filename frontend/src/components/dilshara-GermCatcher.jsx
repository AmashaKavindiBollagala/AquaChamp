...
function FloatingGerm({ option, index, onClick, state, position }) {
  ...
  const animClass = state === "correct" ? "germ-pop" : state === "wrong" ? "germ-bounce" : "germ-float";
    ...
    <div style={styles.fullScreen("#0a1628")}>
      ...
      <div style={styles.introCard}>
        ...
        <p style={{ color: "#cbd5e1", fontSize: 14, margin: "0 0 20px", textAlign: "center", maxWidth: 300 }}>
          Tap the germ bubble with the <strong>correct answer</strong> to catch it!
        </p>
  ...
  return (
    <div style={styles.fullScreen("#0a1628")}>
      ...
      <div style={{
        ...
        borderRadius: 20, border: "1px solid #1e3a5f", overflow: "hidden", minHeight: 280,
      }}>
        <div style={{ position: "absolute", inset: 0, overflow: "hidden", opacity: 0.15 }}>
          {[...Array(6)].map((_, i) => (
            <div key={i} style={{
              position: "absolute", width: 80 + i * 20, height: 80 + i * 20,
              borderRadius: "50%", background: GERM_COLORS[i % GERM_COLORS.length],
              left: `${10 + i * 15}%`, top: `${20 + (i % 3) * 25}%`, filter: "blur(20px)",
            }} />
          ))}
        </div>
...
const styles = {
  fullScreen: (bg) => ({ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", fontFamily: "'Nunito', 'DM Sans', sans-serif" }),
  introCard: { margin: "auto", maxWidth: 420, width: "100%", padding: 32, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" },
  avatar: (size) => ({ width: size, height: size, borderRadius: "50%", border: "3px solid #3b82f6", background: "#0f2040" }),
  infoPills: { display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginBottom: 20 },
  pill: (color) => ({ padding: "4px 12px", borderRadius: 20, background: color + "22", border: `1px solid ${color}`, color, fontSize: 12, fontWeight: 700 }),
  bigBtn: (color) => ({ padding: "14px 32px", background: color, color: "#fff", fontSize: 16, fontWeight: 800, border: "none", borderRadius: 16, cursor: "pointer", fontFamily: "'Nunito', sans-serif", boxShadow: `0 4px 20px ${color}66` }),
  header: { display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #1e3a5f" },
};
